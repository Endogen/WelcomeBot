...
import welcomebot.utils as utl
import welcomebot.constants as con

from importlib import reload
from zipfile import ZipFile
from welcomebot.config import ConfigManager
...
class TelegramBot:
    ...
    def __init__(self, config: ConfigManager, token):
        ...
        download = MessageHandler(Filters.document, self._update_plugin)
        self.dispatcher.add_handler(download)

        join = MessageHandler(Filters.status_update.new_chat_members, self._user_joined)
        self.dispatcher.add_handler(join)
        ...
        for admin in config.get("admin", "ids"):
            try:
                self.updater.bot.send_message(admin, "WelcomeBot is up and running!")
            except Exception as e:
                logging.warning(f"Couldn't send 'up and running' message to {admin}")
    ...
    def _user_joined(self, bot, update):
        path = os.path.join(con.DIR_RES, "welcome.md")
